    ...
    <div className="flex flex-col items-center text-center py-12 md:py-16 relative">
      ...
      <div className="max-w-4xl w-full space-y-6 relative z-10 flex flex-col items-center">
        ...
        <div className="relative w-28 h-28 md:w-32 md:h-32 group mb-4">
          ...
          <div className={`w-full h-full rounded-full overflow-hidden border-2 ${isDarkMode ? 'border-navy-light' : 'border-white'} shadow-xl relative z-10 flex items-center justify-center bg-navy`}>
            <img 
        ...
        {/* Name */}
        <h1 className={`text-5xl md:text-8xl font-heading font-extrabold tracking-tight ${isDarkMode ? 'text-white' : 'text-navy'}`}>
          David Mitrani
        </h1>

        {/* Secondary Title & Typewriter */}
        <div className="space-y-1">
          <p className={`text-lg md:text-2xl font-heading font-semibold ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
            <span className="text-cyber">Mastering</span> Data & <span className="text-violet-500">Financial Analysis</span>
          </p>
          <div className={`text-base md:text-xl font-mono h-6 ${isDarkMode ? 'text-slate-300' : 'text-slate-500'}`}>
            {text}<span className="text-cyber animate-pulse">|</span>
          </div>
        </div>

        {/* Narrative Description */}
        <p className={`max-w-xl text-sm md:text-base font-light leading-relaxed ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center space-y-3 sm:space-y-0 sm:space-x-4 pt-4">
          ...
          <a
            ...
            className={`w-full sm:w-auto px-6 py-3 border-2 border-cyber text-cyber font-bold rounded-lg transition-all hover:bg-cyber/10 transform hover:-translate-y-1 text-center text-sm ${isDarkMode ? '' : 'bg-white'}`}
          >
      ...
      <div className="mt-12 animate-bounce opacity-20 hover:opacity-100 transition-opacity cursor-pointer" onClick={() => document.getElementById('global-advantage')?.scrollIntoView()}>
        <svg xmlns="http://www.w3.org/2000/svg" className={`h-5 w-5 ${isDarkMode ? 'text-white' : 'text-navy'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
